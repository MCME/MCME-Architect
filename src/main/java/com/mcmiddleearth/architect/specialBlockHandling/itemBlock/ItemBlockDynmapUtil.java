...
public class ItemBlockDynmapUtil {
    ...
    public static void createMarker(ItemBlockRegion region) {
        if(!enabled) {
            return;
        }
        if(!init) {
    ...
    DevUtil.log("Adding Dynmap AreaMarker for region: " + region.getName());
            ...
            areaMarker.setDescription(region.getName()+": Limit: "+region.getLimit());
        }
    }
    
    private static double[] getXPoints(ItemBlockRegion region) {
    ...
    private static double[] getZPoints(ItemBlockRegion region) {
        if(region.getRegion() instanceof Polygonal2DRegion) {
            double[] result = new double[((Polygonal2DRegion)region.getRegion()).getPoints().size()];
            for(int i = 0; i < result.length; i++) {
                BlockVector2 vector = ((Polygonal2DRegion)region.getRegion()).getPoints().get(i);

...
| H | Per-event cost & memory leaks | Busy build sessions over long uptime | Tick lag, slow heap growth | ✅ Leaks fixed (perf = cut) |
...
- ✅ **Thread-safety corrections (concurrency core)** *(done on `architect-rework-2026`, Wave C — commits `a5dd1a7`…`ca812ac`)* — `/inv download|upload` and RP-release callbacks now run on the main thread (were reloading inventories / messaging off-thread) and their leaked `newSingleThreadExecutor`s are shut down; `/chead submit` now does the whole Mojang HTTP round-trip off-thread and hops only the Bukkit result to main (dropping the main-thread response-read stall and the non-volatile `received`/`connection` race); `playerRpData` (written from `AsyncPlayerPreLogin`) + `sodiumClients` are now `ConcurrentHashMap` and cleared on quit. Independently reviewed — the review caught, and this slice then fixed, a `ConcurrentHashMap`-vs-null regression the map change introduced in an untouched caller (`RpDatabaseConnector` used `put(uuid, null)` as a load-failed marker, which would have NPE'd on the async DB thread on every DB hiccup). **Follow-ups — all now done in later Wave C slices:** the per-player memory-leak maps (`CopyPasteManager`, `BlockCycleListener`), `RpManager.refreshSHA` (stream leak + off-thread config/message), and `EntityLogger` async CME are fixed in the **hygiene slice**; the newly-found `RpDatabaseConnector` eager async keep-alive is guarded in the **data-layer slice** (`dbConfigured`). (Risk F). *M*
- ✅ **Per-player state hygiene** *(done, Wave C)* — `playerRpData`/`sodiumClients` made concurrent + cleared on quit (thread-safety slice); `CopyPasteManager` (clipboards/undo/redo) and `BlockCycleListener` (`playerBlockDataManager`) cleared on quit; and — after the review surfaced 5 more command-owned static maps (`WeSelectCommand` weSelect/weSelectShift, `Banner`/`ArmorStandEditorCommand` configList, `SlCommand` stencilLists) — those are now cleared via a new central `PlayerDataCleanupListener`. Independently reviewed (clean on the fixes; the extra maps it found were then also closed). (hygiene slice, commits `3b7273c`…`92e3a10`.) (Risk H, memory half; the per-event-cost/performance half remains an explicit cut-candidate.) *S–M*
